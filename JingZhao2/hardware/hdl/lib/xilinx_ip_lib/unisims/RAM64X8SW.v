...
`endif
  ...
  initial begin
    ...
    mem_h = INIT_H;
    #100;
    O = {mem_a[A], mem_b[A], mem_c[A], mem_d[A], mem_e[A], mem_f[A], mem_g[A], mem_h[A]};
  end
...
`ifdef XIL_TIMING
   always @ (mem_a[A_dly] or A_dly) begin
     if (O[7] !== mem_a[A_dly]) O[7] = mem_a[A_dly];
   end
   always @ (mem_b[A_dly] or A_dly) begin
     if (O[6] !== mem_b[A_dly]) O[6] = mem_b[A_dly];
   end
   always @ (mem_c[A_dly] or A_dly) begin
     if (O[5] !== mem_c[A_dly]) O[5] = mem_c[A_dly];
   end
   always @ (mem_d[A_dly] or A_dly) begin
     if (O[4] !== mem_d[A_dly]) O[4] = mem_d[A_dly];
   end
   always @ (mem_e[A_dly] or A_dly) begin
     if (O[3] !== mem_e[A_dly]) O[3] = mem_e[A_dly];
   end
   always @ (mem_f[A_dly] or A_dly) begin
     if (O[2] !== mem_f[A_dly]) O[2] = mem_f[A_dly];
   end
   always @ (mem_g[A_dly] or A_dly) begin
     if (O[1] !== mem_g[A_dly]) O[1] = mem_g[A_dly];
   end
   always @ (mem_h[A_dly] or A_dly) begin
     if (O[0] !== mem_h[A_dly]) O[0] = mem_h[A_dly];
   end
`else
   always @ (mem_a[A] or A) begin
     if (O[7] !== mem_a[A]) O[7] = mem_a[A];
   end
   always @ (mem_b[A] or A) begin
     if (O[6] !== mem_b[A]) O[6] = mem_b[A];
   end
   always @ (mem_c[A] or A) begin
     if (O[5] !== mem_c[A]) O[5] = mem_c[A];
   end
   always @ (mem_d[A] or A) begin
     if (O[4] !== mem_d[A]) O[4] = mem_d[A];
   end
   always @ (mem_e[A] or A) begin
     if (O[3] !== mem_e[A]) O[3] = mem_e[A];
   end
   always @ (mem_f[A] or A) begin
     if (O[2] !== mem_f[A]) O[2] = mem_f[A];
   end
   always @ (mem_g[A] or A) begin
     if (O[1] !== mem_g[A]) O[1] = mem_g[A];
   end
   always @ (mem_h[A] or A) begin
     if (O[0] !== mem_h[A]) O[0] = mem_h[A];
   end
`endif
