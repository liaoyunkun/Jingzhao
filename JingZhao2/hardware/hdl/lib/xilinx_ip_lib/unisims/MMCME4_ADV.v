...
`ifndef XIL_TIMING
  ...
  localparam real VCOCLK_FREQ_MIN = 800.0;
`endif
...
`endif
  ...
  assign CLKIN2_in = (CLKIN2 !== 1'bz) && (CLKIN2 ^ IS_CLKIN2_INVERTED_BIN); // rv 0
        ...
        if (DWE_in == 1) begin          // write process
          if (rst_input == 1 || CDDCREQ_in == 1) begin
            ...
            if (DADDR_in == 7'd6)
              ...
              if (DI_in[11] == 1'b1) begin
                if ((clk0_ht == 6'h00) && (clk0_lt == 7'h00)) begin
                  clk0_pm_f = 3'b100;
                  clk0_pm_r = 3'b100;
                end
              end
...
// free run vco comp
 ...
 always  @(negedge clk0in or posedge rst_in_o)
  ...
  always @(posedge clkin_osc or posedge rst_int or posedge clkin_p)
      if ((rst_int == 1) || (LOCKED_out == 1'b0)) begin
        clkinstopped_out1 <= 0;
        clkin_lost_cnt <= 0;
      end
...
// determine int and frac part
    ...
    if (frac_en == 1'b1)
      vector = mmcm_frac_calc(div_int,phase*1000,duty*1000,div_frac*1000);
    else
