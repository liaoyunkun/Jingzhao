...
// inputs with no timing checks
  ...
  always @ (trig_attr) begin
    ...
    if ((attr_test == 1'b1) ||
        (RX_REFCLK_FREQUENCY_REG < 200.0 || RX_REFCLK_FREQUENCY_REG > 2667.0)) begin
      $display("Error: [Unisim %s-122] RX_REFCLK_FREQUENCY attribute is set to %f.  Legal values for this attribute are 200.0 to 2667.0. Instance: %m", MODULE_NAME, RX_REFCLK_FREQUENCY_REG);
      attr_err = 1'b1;
    end
    ...
    if ((attr_test == 1'b1) ||
        (TX_REFCLK_FREQUENCY_REG < 200.0 || TX_REFCLK_FREQUENCY_REG > 2667.0)) begin
      $display("Error: [Unisim %s-136] TX_REFCLK_FREQUENCY attribute is set to %f.  Legal values for this attribute are 200.0 to 2667.0. Instance: %m", MODULE_NAME, TX_REFCLK_FREQUENCY_REG);
      attr_err = 1'b1;
    end
