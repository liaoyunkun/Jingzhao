...
`endif
  ...
  integer PER_BIT_FINE_DELAY = 5;
  integer PER_BIT_MEDIUM_DELAY = 40;
  integer INTRINSIC_FINE_DELAY = 75;
  integer INTRINSIC_MEDIUM_DELAY = 40;

  integer ODATAIN_INTRINSIC_DELAY = 60;
  integer CASC_IN_INTRINSIC_DELAY = 60;
  //localparam CASC_RET_INTRINSIC_DELAY = 50;
  integer CASC_RET_INTRINSIC_DELAY = 60;

  integer DATA_OUT_INTRINSIC_DELAY = 25;
  integer CASC_OUT_INTRINSIC_DELAY = 80;
...
// inputs with no timing checks
  ...
  always @ (trig_attr) begin
    ...
    if ((attr_test == 1'b1) ||
        (REFCLK_FREQUENCY_REG < 200.0 || REFCLK_FREQUENCY_REG > 2667.0)) begin
      $display("Error: [Unisim %s-108] REFCLK_FREQUENCY attribute is set to %f.  Legal values for this attribute are 200.0 to 2667.0. Instance: %m", MODULE_NAME, REFCLK_FREQUENCY_REG);
      attr_err = 1'b1;
    end
...
begin
  ...
  begin
    if (DELAY_VALUE_REG == 0)
      gen_mc_fixed_dly_ratio = 18'b0;
    else
      gen_mc_fixed_dly_ratio = (2**9) * ( ( (2 * 1000000) / REFCLK_FREQUENCY_REG ) / DELAY_VALUE_REG);
  end
  else if (DELAY_FORMAT_REG == "COUNT")
  begin
    gen_mc_fixed_dly_ratio = {9'b0, DELAY_VALUE_REG};
  end
end */
	...
	case(DELAY_TYPE_REG)
			...
			"VAR_LOAD":
				...
				casex({LOAD_in, CE_in, INC_in})
					...
					3'b110: 
						if(UPDATE_MODE_BIN != UPDATE_MODE_MANUAL) $display("Error: [Unisim %s-2] Invalid scenario. LOAD = 1, CE = 1 INC = 0 is not valid for UPDATE_MODE=%s and DELAY_TYPE=%s. Instance: %m", MODULE_NAME, UPDATE_MODE_REG,DELAY_TYPE_REG);
						else cntvalue_updated_async <= idelay_count_async;
					3'b111: 
						if(UPDATE_MODE_BIN != UPDATE_MODE_MANUAL) $display("Error: [Unisim %s-3] Invalid scenario. LOAD = 1, CE = 1 INC = 0 is not valid for UPDATE_MODE=%s and DELAY_TYPE=%s. Instance: %m", MODULE_NAME, UPDATE_MODE_REG,DELAY_TYPE_REG);
