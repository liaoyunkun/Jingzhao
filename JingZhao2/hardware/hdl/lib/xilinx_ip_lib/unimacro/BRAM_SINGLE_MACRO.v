// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/unimacro/data/devlib/verilog/BRAM_SINGLE_MACRO.v,v 1.27 2012/11/01 23:02:50 vandanad Exp $
///////////////////////////////////////////////////////////////////////////////
...
module BRAM_SINGLE_MACRO (DO,  
   ...
   initial
     begin
       ...
       if (DEVICE == "VIRTEX5" || DEVICE == "VIRTEX6" || DEVICE == "7SERIES")
	    ...
	    if( (READ_WIDTH != WRITE_WIDTH) && (READ_WIDTH/WRITE_WIDTH !=2) && (WRITE_WIDTH/READ_WIDTH !=2) && (valid_width == "FALSE")) 
            begin
              $display("Attribute Syntax Error : The attributes READ_WIDTH, WRITE_WIDTH on BRAM_SINGLE_MACRO instance %m are set to %d, %d.  To use BRAM_SINGLE_MACRO, one of the following conditions must be true-\n 1. READ_WIDTH must be equal to WRITE_WIDTH \n 2. If assymetric, READ_WIDTH and WRITE_WIDTH must have a ratio of 2.\n 3. If assymetric, READ_WIDTH and WRITE_WIDTH should have values 1, 2, 4, 8, 9, 16, 18, 32, 36, 64, 72.", READ_WIDTH, WRITE_WIDTH);
              #1 $finish;
            end
            if (READ_WIDTH == 0)
              begin 
  ...
  localparam DO_WIDTH = (DEVICE == "VIRTEX5" || DEVICE == "VIRTEX6" || DEVICE == "7SERIES" || DEVICE == "SPARTAN6" ) ? ( (READ_WIDTH == 1) ? 1 : (READ_WIDTH == 2) ? 2 : (READ_WIDTH > 2 && READ_WIDTH <= 4) ? 4 : (READ_WIDTH > 4 && READ_WIDTH <= 9) ? 8 : (READ_WIDTH > 9 && READ_WIDTH <= 18) ? 16 : (BRAM_SIZE == "18Kb" && READ_WIDTH > 18 && READ_WIDTH <= 36) ? (READ_WIDTH-DOP_WIDTH) : (BRAM_SIZE == "36Kb" && READ_WIDTH > 18 && READ_WIDTH <= 36) ? 32 : (BRAM_SIZE == "36Kb" && READ_WIDTH > 36 && READ_WIDTH <= 72) ? (READ_WIDTH-DOP_WIDTH) : (READ_WIDTH-DOP_WIDTH) ): (READ_WIDTH-DOP_WIDTH);
   ...
   localparam RD_BYTE_WIDTH = (DEVICE == "VIRTEX5" || DEVICE == "VIRTEX6" || DEVICE == "7SERIES" || DEVICE == "SPARTAN6" ) ? ( (READ_WIDTH <= 9) ? 1 : (READ_WIDTH > 9 && READ_WIDTH <= 18) ? 2 : (READ_WIDTH > 18 && READ_WIDTH <= 36) ? 4 : (READ_WIDTH > 36 && READ_WIDTH <= 72) ? 8 : (BRAM_SIZE == "9Kb" || BRAM_SIZE == "18Kb") ? 2 : 4 ) : 4;
   ...
   generate
     case(DEVICE)
       "VIRTEX5", "VIRTEX6", "7SERIES": 
          begin
	    if ((WRITE_P == "TRUE" && READ_P == "TRUE") && ( (BRAM_SIZE == "18Kb" && WRITE_WIDTH == 36) || (BRAM_SIZE == "36Kb" && WRITE_WIDTH == 72) ))
            begin : di11
		 ...
		 for (i = 0; i < WE_WIDTH; i = i + 1) begin : di_byte1
		  ...
		  assign DI_PATTERN = {DI[69:54], DI[52:45], DI[43:36], DI[34:27], DI[25:18],  DI[16:9], DI[7:0]};
	       ...
	       for (j = 1; j <= WR_WIDTHP; j = j + 1) begin : dip_byte1
            ...
            else if ((WRITE_P == "TRUE" && READ_P == "TRUE") && ( (BRAM_SIZE == "18Kb" && WRITE_WIDTH < 36) || (BRAM_SIZE == "36Kb" && WRITE_WIDTH < 72) ))
            begin : di12
		 ...
		 assign DIP_PATTERN[j-1] = DI[((j*8)+j-1)];
	       ...
	       for (j2 = DIP_WIDTH; j2 < MAX_DIP_SIZE; j2 = j2 + 1) begin : dip_byte2_0
		  assign DIP_PATTERN[j2] = 1'b0;
	       end
	       
            end  
            else if ((WRITE_P == "FALSE" && READ_P == "FALSE") && ( (BRAM_SIZE == "18Kb" && WRITE_WIDTH == 32) || (BRAM_SIZE == "36Kb" && WRITE_WIDTH == 64) ))
            begin : di13
              assign DIP_PATTERN = 0;
              assign DI_PATTERN = DI;
            end
            else if ((WRITE_P == "FALSE" && READ_P == "FALSE") && ( ((READ_WIDTH == WRITE_WIDTH) || (READ_WIDTH/WRITE_WIDTH == 2) || (valid_width == "TRUE"))) )
            begin : di14
              assign DIP_PATTERN = 0;
		...
		begin
         ...
         "SPARTAN6" : 
          begin
	    if((WRITE_P == "TRUE" && READ_P == "TRUE") && ( (BRAM_SIZE == "9Kb" && WRITE_WIDTH == 36) || (BRAM_SIZE == "18Kb" && WRITE_WIDTH == 72) ))
              begin : di21
		 
	       if (WRITE_WIDTH >= 71 || WRITE_WIDTH == 35 || WRITE_WIDTH == 36 || WRITE_WIDTH <= 32) begin : di221
		 for (i = 0; i < WE_WIDTH; i = i + 1) begin : di21_byte1
		    assign DI_PATTERN[(i*8) +: 8] = DI[((i*8)+i) +: 8];
		 end
	       end
		  ...
		  assign DI_PATTERN = {DI[69:54], DI[52:45], DI[43:36], DI[34:27], DI[25:18],  DI[16:9], DI[7:0]};
	       ...
	       for (j = 1; j <= WR_WIDTHP; j = j + 1) begin : dip21_byte1
	          assign DIP_PATTERN[j-1] = DI[((j*8)+j-1)];
	       end
		 
            end
            else if ((WRITE_P == "TRUE" && READ_P == "TRUE") && ( (BRAM_SIZE == "9Kb" && WRITE_WIDTH < 36) || (BRAM_SIZE == "18Kb" && WRITE_WIDTH < 72) ))
            begin : di22

	       if (WRITE_WIDTH >= 71 || WRITE_WIDTH == 35 || WRITE_WIDTH == 36 || WRITE_WIDTH <= 32) begin : di221
		 for (i = 0; i < WE_WIDTH; i = i + 1) begin : di22_byte2
		    assign DI_PATTERN[(i*8) +: 8] = DI[((i*8)+i) +: 8];
		 end
	       end
		  ...
		  assign DI_PATTERN = {DI[69:54], DI[52:45], DI[43:36], DI[34:27], DI[25:18],  DI[16:9], DI[7:0]};
	       ...
	       for (j = 1; j <= WR_WIDTHP; j = j + 1) begin : dip22_byte2
		 assign DIP_PATTERN[j-1] = DI[((j*8)+j-1)];
	       end

	       for (j2 = DIP_WIDTH; j2 < MAX_DIP_SIZE; j2 = j2 + 1) begin : dip22_byte2_0
		  assign DIP_PATTERN[j2] = 1'b0;
	       end
	       
            end
            else if ((WRITE_P == "FALSE" && READ_P == "FALSE") && ( (BRAM_SIZE == "9Kb" && WRITE_WIDTH == 36) || (BRAM_SIZE == "18Kb" && WRITE_WIDTH == 72) ))
            begin : di23
              assign DI_PATTERN = DI;
            end
            else if ((WRITE_P == "FALSE" && READ_P == "FALSE") && ( ((READ_WIDTH == WRITE_WIDTH) || (READ_WIDTH/WRITE_WIDTH == 2) || (valid_width == "TRUE"))) )
   ...
   generate 
     case(DEVICE)
       "VIRTEX5", "VIRTEX6", "7SERIES", "SPARTAN6": 
          if ( ((DEVICE == "VIRTEX5" || DEVICE == "VIRTEX6" || DEVICE == "7SERIES") && (BRAM_SIZE == "18Kb" && READ_WIDTH <= 36) || (BRAM_SIZE == "36Kb" && READ_WIDTH <= 72) ) || ( (DEVICE == "SPARTAN6") && (BRAM_SIZE == "9Kb" && READ_WIDTH <= 36) || (BRAM_SIZE == "18Kb" && READ_WIDTH <= 72) ) )
	    begin : do1
     ...
     case (DEVICE)
       "VIRTEX5" : 
          ...
          RAMB18 # ( 
            ...
            .INIT_3E (INIT_3E),
            .INIT_3F (INIT_3F),
            .INIT_A ({init_tmp[35:34],init_tmp[31:16]}),
            .INIT_B ({init_tmp[33:32],init_tmp[15:0]}),
            .INIT_FILE (INIT_FILE),
            .READ_WIDTH_A (18),
            .READ_WIDTH_B (18),
            .SIM_COLLISION_CHECK ("NONE"),
            .SIM_MODE (SIM_MODE),
            .SRVAL_A ({srval_tmp[35:34],srval_tmp[31:16]}),
            .SRVAL_B ({srval_tmp[33:32],srval_tmp[15:0]}),
            .WRITE_MODE_A (WRITE_MODE),
            ...
            .ADDRA({ADDR,1'b0,4'b1111}), 
            .ADDRB({ADDR,1'b1,4'b1111}), 
            .CLKA (CLK), 
            .CLKB (CLK), 
          ...
          RAMB36 # ( 
            ...
            .INIT_7E (INIT_7E),
            .INIT_7F (INIT_7F),
            .INIT_A ({init_tmp[71:68],init_tmp[63:32]}),
            .INIT_B ({init_tmp[67:64],init_tmp[31:0]}),
            .INIT_FILE (INIT_FILE),
            .READ_WIDTH_A (36),
            .READ_WIDTH_B (36),
            .SIM_COLLISION_CHECK ("NONE"),
            .SIM_MODE (SIM_MODE),
            .SRVAL_A ({srval_tmp[71:68],srval_tmp[63:32]}),
            .SRVAL_B ({srval_tmp[67:64],srval_tmp[31:0]}),
            .WRITE_MODE_A (WRITE_MODE),
            .WRITE_MODE_B (WRITE_MODE),
            .WRITE_WIDTH_A (36),
            .WRITE_WIDTH_B (36)
        ...
        "VIRTEX6", "7SERIES" : 
          begin
            ...
            .ADDRARDADDR({ADDR,1'b0,4'b1111}), 				   
	    .ADDRBWRADDR({ADDR,1'b1,4'b1111}), 
            .CLKARDCLK (CLK), 
            .CLKBWRCLK (CLK), 
